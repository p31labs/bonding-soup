...
| **Universal UI (one sky)** | **`docs/P31-UNIVERSAL-UI-VISION.md`** — single night sky, twenty-three windows; **altitude** (magnification) not “different products”; five minds as reading orders; Layer 1.5 starfield; **return ribbon** (`soup` · `hub` · `passport` · `connection` · `mesh`); static star plate for breaker panels (command center, operator desk). Normative layout remains **`docs/P31-DESIGN-DOCTRINE.md`**; tokens **`andromeda/04_SOFTWARE/design-tokens/p31-universal-canon.json`**; **`p31-alignment.json`** source **`p31-universal-ui-vision-md`**. **Speakable vocabulary (DELTA):** **`docs/P31-DELTA-LANGUAGE.md`**, machine registry **`docs/p31-delta-language.json`**, glossary **`docs/p31-delta-glossary.html`** / hub **`/delta-language.html`**; **`verify:delta-language`**. |
...
**K₄ / SIC-POVM metaphor, vibe+agentic workflow, youth path:** **`docs/SIC-POVM-K4-ARCHITECTURE.md`**, **`docs/SIC-POVM-MATHEMATICAL-APPENDIX.md`** (definitions + d = 2 Bloch tetrahedron), **`docs/AGENTIC-VIBE-INFRASTRUCTURE.md`**, **`docs/PLAN-KIDS-VIBE-CODING.md`**. **Ethical UI (P31 token semantics, motion, “rewards” — no dark patterns; agent checklist):** **`docs/ETHICAL-STYLE-MAP.md`**. **Universal visual constitution (one sky, many windows — altitude, ribbon, starfield Layer 1.5):** **`docs/P31-UNIVERSAL-UI-VISION.md`**. **DELTA language (speakable canon — anchors, terms, forbidden phrases; CI `verify:delta-language`):** **`docs/P31-DELTA-LANGUAGE.md`**, **`docs/p31-delta-language.json`**, **`docs/p31-delta-glossary.html`**. **Quantum egg + Larmor UI coherence (CI: `verify:egg-hunt`):** **`docs/EGG-HUNT.md`**, manifest **`docs/egg-hunt-manifest.json`**. 

**External model / handoff review (Gemini, Opus, etc.):** start at **`docs/README-REVIEW-DOCS.md`** — index to **`docs/GEMINI-OPUS-REVIEW-BUNDLE.md`**, workflow / Workers / CWP supplements, **`docs/MVP-DELIVERABLES-INVENTORY.md`** (tiered LIVE/MVP + grant summary), and **`docs/P31-MARKET-LAUNCH-PACKAGE.md`** (market launch + GTM spine). **Human onboarding (Tyler / collaborators):** **`docs/HANDOFF-TYLER-P31.md`** — live URLs, both repos, PR pointer, verify commands, operator triangle. **Updated 2026-04-25:** see **`p31-constants.json`** for operator-locked numbers; **`docs/CANONICAL-NUMBERING.md`** for namespaces. **`npm run apply:constants`** / **`verify:constants`** keep `ground-truth` aligned.
