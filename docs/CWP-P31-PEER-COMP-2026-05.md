...
| 11 | Accessibility | Standard | Industry-leading per Apple marketing (VoiceOver, etc.) | Standard | Strong (named team) | `screenComfort` cascade in CogPass; partial WCAG | **Phase 1 — WCAG 2.2 AA audit + remediation** (PEER-1G) | Accessibility as afterthought |
...
4. **The DELTA language + ethical style map.** A documented voice with an avoid-list that forbids urgency manipulation, marketing-tier "unlock" framing, naval/military metaphors, and engagement-maximization patterns. Verified at CI time (`verify:delta-language`, `verify:public-voice`). Mozilla has a Style Guide; P31 has a *measurable* style guide.
...
### Phase 1 — Trust & transparency layer (highest impact, ~2 agent-weeks)
