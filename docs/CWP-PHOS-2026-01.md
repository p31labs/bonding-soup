...
| 25 | `2fd0aa8` | home | main | qa(D-7): full-verify pass — schemas extension + Tier-A sanitization + CWP ledger |
| 26 | `2fe89dec` | andromeda | pr/fix-broken-hrefs-release-gate | feat(p31ca): nonprofit canon (9 contracts) + grants page + stylebook bug sweep (3 false positives + 1 latent browser-parsing bug) |
| 27 | `2c7bb77` | home | main | feat(nonprofit): operations system (master doc + 7 runbooks + 5 scripts) + alignment registration (5 sources) + launch-readiness fix |
| 28 | `44516b3b1` | andromeda | pr/fix-broken-hrefs-release-gate | feat(e2e): SENTINEL:PASSKEY — 6th persona spec for the user sentinel swarm (203 lines, contract-level WebAuthn + hardware pair) |
| 29 | `e295eef`   | home | main | docs(CWP-PHOS): ledger §10 — nonprofit ops + stylebook bug sweep + SENTINEL:PASSKEY |
| 30 | `4ec470c`   | home | main | feat(verify): bus-bar + cogpass-reader verifiers + TRIPER fixes — release:public unblocked |
| 31 | `36b4fca86` | andromeda | pr/fix-broken-hrefs-release-gate | fix(bus-bar): drop stale 'command-center' productSlots entry (caught by new home-side verifier) |
...
11. **Release ladder unblocked — bus-bar + cogpass-reader verifiers + TRIPER fixes** (commits 29 → 31, 2026-05-02 00:50–01:09) — `release:public` was blocked on two pre-existing TRIPER suites that have been red since the C-4 atomic schema bump weeks ago: PERSONAL had a stale `expect(...).toBe("p31.cognitivePassport/1.0.0")` assertion and SYSTEMS INTEGRITY caught two p31-alignment derivations whose `verify` fields said `npm run verify:X (planned)` for scripts that didn't exist (cogpass-reader + bus-bar). Both gaps closed by SHIPPING the real verifiers — the only durable cure. `scripts/verify-bus-bar.mjs` enforces six structural rules on the busBar block (productSlots ↔ hub-app-ids ∪ subdomain allowlist; productSlots → slots; navByRole → slots; cage `maxProducts == 9`; strict role nesting `stranger ⊂ user ⊂ operator`; `phosVoiceKey` resolves under the same normalization the runtime PHOS guide uses — strip trailing slash + `.html`). `scripts/verify-cogpass-reader.mjs` enforces six structural + behavioral guards on the activator (node --check; full public API `window.p31CogPass.{get,set,clear,getRole,getDisplayName,onChange}`; all three documented events `p31:cogpass-{loaded,cleared,error}`; screenComfort cascade thresholds `30` and `10` with `p31-gray-rock` pin; `STORAGE_KEY` matches privacy.html §2f legal contract; schema URI matches constants). Both verifiers are partial-clone-friendly (skip with note when andromeda missing) and have strict modes (`P31_VERIFY_*_STRICT=1`) for CI on full checkouts. **Bus-bar verifier caught one real bug on first run**: stale `command-center` entry in productSlots — there is no `command-center` product card in the hub registry; the product is `mission-control` (kids token economy, different thing) and the local Node command-center isn't a hub product. Fixed in andromeda companion commit `36b4fca86`. Both anti-drift gates self-triggered cleanly: verifyPipeline.scripts mirror 72 → 74, wiring CI ladder 73 → 75. **Verification:** `npm run release:check` exit 0; `npm run test:triper:cert` 12/12 suites PASS, **108 mutation sentinels GREEN** (was 107/108), **gateStatus AUTHORIZED**, fresh cert at `2026-05-02T05:05:46Z`. **`release:public` is now unblocked** (cert age <24h gate satisfied).

10. **Nonprofit operations system + stylebook bug sweep + SENTINEL:PASSKEY** (commits 25 → 28, 2026-05-02 00:14–00:50) — closing four parallel queues that had been quietly accumulating. **Nonprofit (HOME 2c7bb77 + ANDROMEDA 2fe89dec):** Master operator-facing doc `docs/P31-NONPROFIT-OPERATIONS.md` is the single navigation point at the 9 ground-truth contracts (governance / compliance / financials / donor-policy / programs / grants / legal / risk / people, all under `p31.nonprofit.*/1.0.0`), 7 spoon-deficit-aware runbooks (board-meeting · donor-receipt · filing-calendar · grant-application · incident-response · onboarding · README), and 5 npm scripts (verify · status · calendar · receipt · report). `verify:nonprofit` enforces EIN consistency, separation-of-duties, Charity Navigator floor on programServices, IRS R-NNN risk IDs, board↔roster reciprocity. Public transparency surface `src/pages/grants.astro` ships at `/grants` (BaseLayout + BusBarNav adopter; 7 active + 3 archived entries; companions the open-books policy in `nonprofit/financials.json`). Five new alignment sources registered (217 → 222). **Stylebook (ANDROMEDA 2fe89dec):** Three "broken link" failures from `verify-internal-hub-links.mjs` were the visible symptom of a latent browser-parsing bug — example HTML inside JS template literals contained literal `</script>` tokens that closed the *outer* `<script type="module">` early in browser tokenization, silently breaking page render in production. Two-layer fix: source escapes (5 instances of `</script>` → `<\/script>` across `metadata.html` + `index.html` + `performance.html`) restore proper parsing; verifier now strips `<script>`, `<style>`, `<pre>`, `<code>`, and HTML comments before scanning so example HTML in any doc page can never trigger false positives again. **291 hub html files scan clean** (was 3 false positives + 1 silent prod bug). **Launch readiness (HOME 2c7bb77):** `scripts/p31-launch-readiness.mjs` template literal had a backtick collision parsing `` `npm run launch:audit` `` as code — escaped to `<code>` block; broken anchor at `/tmp/...json` (not publicly served) replaced with literal text reference. **SENTINEL:PASSKEY (ANDROMEDA 44516b3b1):** 6th sibling of the user-sentinel swarm — was authored but never committed since 4f98f0ae1. 203 lines of contract-level WebAuthn + hardware-pair tests (`/api/passkey/*` + `/api/hardware/*`), graceful skip on Astro preview where workers aren't mounted, asserts no `sk_live_` / `pk_live_` / `Bearer` token leaks in passport-generator HTML. **Full root `npm run verify` GREEN exit 0 (72 gates). Postbuild link verifier OK (291 html files scanned).**
