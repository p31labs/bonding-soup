...
- **C.A.R.S. dev:** root **`soup.html`** (with **`npm run demo`**) links to **`andromeda/04_SOFTWARE/p31ca/public/planetary-onboard.html`** and **`mesh-start.html`** so the engine demo and PAR front door share one local static server.  
