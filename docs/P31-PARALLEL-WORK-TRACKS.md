...
|-------|--------|-----------|-------------------|
| **Soup product** | Home root — **`soup.html`**, **`src/`**, **`soup-quantum.css`** | **`npm run verify`**, **`npm run soup:prep:check`** | Perf, sim, canvas; **not** hub Workers |
| **p31ca hub** | **`andromeda/04_SOFTWARE/p31ca/`** | **`npm run hub:ci`** (when tree present) | Registry, ground-truth, Astro, **Workers** in allowlist |
| **Bonding deploy** | **`npm run sync:soup-bonding`** → bonding vertical | After Soup milestone + **`apply:constants`** | Thin sync PRs |
...
npm run sync:soup-bonding   # After Soup surface changes (deploy path)
```
...
- **`docs/CWP-SOUP-MVP-SEQUENCE-2026-01.md`** — Soup sequencing vs when-scale.
