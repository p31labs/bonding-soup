...
- **`DRAFT-AGENT-SIMULATED`** — stand-in copy generated by the Architect under §2 voice rules (avoid-list aware) to keep PHOS speakable on every page until the operator extends. Agents may improve a draft (still tagged DRAFT). The operator promotes by rewriting and changing the tag to `OPERATOR-VOICE`.
