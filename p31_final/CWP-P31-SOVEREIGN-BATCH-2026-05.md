...
**Spoon estimate:** 3 🥄🥄🥄 (agent spoons, not operator)
...
**Why design-only:** The implementation requires UF Health MyChart OAuth credentials that the operator doesn't have yet. The agent can design the entire system, document the API flows, and write the TypeScript skeleton — but can't test against the real API until the operator completes the meatspace step (see below).
