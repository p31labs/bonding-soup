 ...
 * cognitive-passport/index.html (per p31ca/scripts/passport-p31ca-transform).
 * Run: npm run verify:passport
 */
import fs from "fs";
...
async function main() {
  ...
  if (!mod || typeof mod.toP31caMirror !== "function") {
    console.log(
      "verify-passport: skip — passport transform not available (partial clone):",
      canonicalPassportTransformPath
    );
    process.exit(0);
  }
  ...
  let expected;
  try {
    expected = mod.toP31caMirror(fs.readFileSync(src, "utf8"));
  } catch (e) {
